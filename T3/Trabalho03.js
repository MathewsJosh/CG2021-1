...
function cria_afuselagem(ponto) {
        ...
        leme_dir.name = 'Sombra';
        leme_esq.name = 'Sombra';
        leme_cima.name = 'Sombra'; 
        leme_dir.castShadow = false;
        leme_esq.castShadow = false;
        leme_cima.castShadow = false; 
        rabeta.add(leme_dir);
        rabeta.add(leme_esq);
        ...
        asa_prancha.name = 'Sombra';
        asa_prancha.castShadow = false;
        // Cima
        var asa_cima = new THREE.Mesh();
        cabine.add(asa_cima);
        asa_cima.position.set(0, 1, cabine_geometria.parameters.radiusTop + 2.2);
        asa_cima.name = 'Sombra';
        asa_cima.castShadow = false;
        ...
        asa_baixo.name = 'Sombra';
        asa_baixo.castShadow = false;
        ...
        cilindro_esq_asa_baixo.name = 'Sombra';
        cilindro_esq_asa_baixo.castShadow = false;
        cilindro_dir_asa_baixo.name = 'Sombra';
        cilindro_dir_asa_baixo.castShadow = false;
        // Direito
        ...
        protecao.name = 'Sombra';
        protecao.castShadow = false;
        cone_motor.name = 'Sombra';
        cone_motor.castShadow = false;
        pa01.name = 'Sombra';
        pa01.castShadow = false;
        pa02.name = 'Sombra';
        pa02.castShadow = false;
        pa03.name = 'Sombra';
        pa03.castShadow = false;
        pa04.name = 'Sombra';
        pa04.castShadow = false;
        ...
        flap_asa_direita_baixo.name = 'Sombra';
        flap_asa_direita_baixo.castShadow = false;
        ...
        flap_asa_esquerda_baixo.name = 'Sombra';
        flap_asa_esquerda_baixo.castShadow = false;
        ...
        asa_meio_baixo.name = 'Sombra';
        asa_meio_baixo.castShadow = false;
        ...
        asa_frente_baixo.name = 'Sombra';
        asa_frente_baixo.castShadow = false;
        ...
        flap_leme_dir.name = 'Sombra';
        flap_leme_dir.castShadow = false;
        ...
        flap_leme_esq.name = 'Sombra';
        flap_leme_esq.castShadow = false;
        ...
        flap_leme_cima.name = 'Sombra';
        flap_leme_cima.castShadow = false;
        ...
        leme_dir_meio.name = 'Sombra';
        leme_dir_meio.castShadow = false;
        ...
        leme_dir_frente.name = 'Sombra';
        leme_dir_frente.castShadow = false;
        ...
        leme_esq_meio.name = 'Sombra';
        leme_esq_meio.castShadow = false;
        ...
        leme_esq_frente.name = 'Sombra';
        leme_esq_frente.castShadow = false;
        ...
        leme_cima_meio.name = 'Sombra';
        leme_cima_meio.castShadow = false;
...
/**
 * Método para carregar objetos externos
 * @param {*} modelPath caminho do objeto
 * @param {*} modelName nome do objeto
 * @param {*} position posição do objeto
 * @param {*} desiredScale escala do objeto importado
 * @param {*} angle1 rotação no eixo x em graus
 * @param {*} angle2 rotação no eixo y em graus
 * @param {*} angle3 rotação no eixo z em graus
 * @param {*} visibility booleano para visibilidade do objeto
 * @param {*} texture booleano para uso de textura
 * @param {*} texturePath caminho da textura
 */
function loadOBJFile(modelPath, modelName, position, desiredScale, angle1, angle2=0, angle3=0, visibility=true, texture=false, texturePath="")
{
        ...
        mtlLoader.load( modelName + '.mtl', function ( materials ) {
                ...
                objLoader.load( modelName + ".obj", function ( obj ) {
                        ...
                        if(texture)
                                obj.traverse( function( node ){ if( node.material ) node.material.map = texturePath ;});
...
/**
 * Método para criar as bases dos prédios (considere que cada base é um paralelepipedo menor que compoe a arquitetura do prédio)
 * @param {*} params parâmetros definidos na função seguinte, onde é passada a textura e tamanhos das paredes
 * @param {*} tipo_teto string que representa o tipo de teto daquele prédio
 * @param {*} rotation rotação das paredes da base
 * @returns objeto prédio
 */
...
/**
 * Método para criar novos predios de acordo com o tipo
 * @param {*} base posição onde o prédio será criado
 * @param {*} tipo qual prédio será criado (escolha de -2 a 6, excluindo o 0)
 * @param {*} rotation a rotação do prédio
 * @param {*} desiredScale escala do prédio
 * @returns objeto prédio
 */
...
// Criação de grupos de prédios e ruas para permitir a repetição dos mesmos
...
//Grupo que contém a cidade completa (sem a praça)
...
/**
 * Função para criar um array de strings contendo o caminho das imagens do bounding box
 * @param {*} filename nome do arquivo
 * @returns array com os caminhos
 */
...
/**
 * Gera um array das texturas carregadas
 * @param {*} filename nome do arquivo
 * @returns array com os materiais
 */
...
//-------------------------------------------------------------------------------------------------
/**
 * Luz dinâmica que acompanha o avião
 */
...
dynamicLight.shadow.camera.left = -30;
dynamicLight.shadow.camera.right = 30;
dynamicLight.shadow.camera.top = 30;
dynamicLight.shadow.camera.bottom = -30;
dynamicLight.receiveShadow = false;
scene.add(dynamicLight)

/**
 * Faz uma segunda luz acompanhar o avião e projetar sua sombra
 */
function moveLightAndTarget() {
    dynamicLight.shadow.camera.updateProjectionMatrix();     
    dynamicLight.target.position.set( aviao_obj.fuselagem._estacionaria.position.x,aviao_obj.fuselagem._estacionaria.position.y, aviao_obj.fuselagem._estacionaria.position.z);   
    dynamicLight.target.updateMatrixWorld();
    dynamicLight.position.set(aviao_obj.fuselagem._estacionaria.position.x+50, aviao_obj.fuselagem._estacionaria.position.y+50, aviao_obj.fuselagem._estacionaria.position.z+70);
}

/**
 * Faz a sombra projetada acompanhar o avião
 * @param {*} objeto 
 */
function Sombra(objeto){
    for (let i of objeto.children) {
            if (i.children.length > 0) {
                    for (let index = 0; index < i.children.length; index++) {
                            Sombra(i.children[index])
                    }
            }

            if(i.name == 'Sombra' || i.name == 'aviao' )
                    i.castShadow = true
    }
}

/**
 *  Define a iluminação para o modo inspeção
 */
...
/**
 * Função para controlar o som ambiente
 * @param {*} audioListener variável para controlar o som
 * @param {*} audioLoader variável para carregar o audio
 * @returns audio
 */
...
/**
 * Função para controlar o som do avião
 * @param {*} audioListener variável para controlar o som
 * @param {*} audioLoader variável para carregar o audio
 * @returns audio
 */
...
/**
 * Função para controlar o som ao passar no checkpoint
 * @param {*} audioListener variável para controlar o som
 * @param {*} audioLoader variável para carregar o audio
 * @returns audio
 */
...
/**
 * Função para controlar o som de conclusão do percurso
 * @param {*} audioListener variável para controlar o som
 * @param {*} audioLoader variável para carregar o audio
 * @returns audio
 */
...
/**
 * Adiciona todos os objetos da cena a uma variável e os reposiciona
 */
...
function render() {
    ...
    if(isSimulacao){
        ...
        // Trabalho 3 - Controle da iluminação do modo inspeção
        ...
        // Trabalho 1 - Oculta o plano e faz o movimento do avião
        plano.visible = true;
        axesHelper.visible = false;
        movimento();

        // Trabalho 2 - Checkpoints
        verificaCheckpoint();
        ...
        if(firstRender){    //O objetivo desse bloco é transformar as sombras dos objetos em sombras estáticas
            directionalLight.shadow.autoUpdate = false;
            firstRender = false;
            Sombra(aviao_obj.fuselagem._estacionaria)
        }
        moveLightAndTarget()
    }
}
